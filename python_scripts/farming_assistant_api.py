...
def predict():
    ...
    if prediction_type not in {"weather", "rice_yield"}:
        return jsonify({
            "error": "Unsupported or missing prediction_type. Use 'weather' or 'rice_yield'."
        }), 422
    ...
    if prediction_type == "weather":
        confidence = confidence_for(predicted_weather, None, None)
        ...
        return jsonify({
            ...
            "source_type": "Trained Model API",
            "source_name": "Farming AI API weather model",
            "source_credit": "Prediction values came from the trained iClimate weather model.",
    ...
    nested_features_payload = {"features": features_payload}
    rice_input = rice_features(nested_features_payload)
    predicted_yield, yield_uncertainty = rice_prediction_with_uncertainty(rice_input)
    recs = recommendations(features_payload, predicted_weather, predicted_yield)
    generated_warnings = warnings_for(features_payload, predicted_weather, predicted_yield)
