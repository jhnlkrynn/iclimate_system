...
class PredictionService
{
    ...
    public function answer(User $user, string $question): array
    {
        ...
        $targetDate = $this->parseTargetDateFromQuestion($question) ?? PredictionDateValidator::defaultTargetDate();

        if ($message = $this->dateValidator->validate($targetDate)) {
            return $this->validationResponse($intent, $language, $memory, $startedAt, $message, $targetDate);
        }
        ...
        $farmType = $conditions['farm_type'] ?? (string) ($profile?->farm_type ?? FarmerProfile::FARM_TYPE_RAINFED);
        $areaOverride = $numbers['area'] ?? null;
        $manualOverrides = array_filter([
            'rainfall' => $numbers['rainfall'] ?? null,
            'temp_avg' => $numbers['temperature'] ?? $conditions['temperature'] ?? null,
            'humidity' => $numbers['humidity'] ?? null,
            'wind_speed' => $numbers['wind_speed'] ?? null,
            'temp_range' => $numbers['temp_range'] ?? null,
        ...
        $weather = [
            'predicted_weather' => $engineResult['weather_label'],
            'rainfall' => $context['rainfall'],
            'temperature' => $context['temp_avg'],
            'humidity' => $context['humidity'],
            'wind_speed' => $context['wind_speed'],
            'season' => $context['season'],
            'confidence' => data_get($engineResult['weather'], 'confidence.value'),
            'source_type' => 'Trained Model',
            'source_name' => 'iClimate monthly Random Forest model',
            'source_credit' => 'Weather output generated by the trained iClimate Random Forest model for the target month.',
        ];
        ...
        $decision = $this->decisionSupport->evaluate([
            ...
            'predicted_weather' => $weather['predicted_weather'],
        ]);

        $warnings = $this->normalizeWarnings([], $decision);
        $quality = $this->predictionQuality($context, $yield, $apiError);
        ...
        $confidence = $this->adjustedConfidence($yield, $decision, $quality);
        ...
        $groqFallback = $this->groqPredictionFallback($user, $question, $intent, $language, $memory, $context, $weather, $yield, $decision, $warnings, $quality, $apiError);
        ...
        return [
            ...
            'source_name' => $groqFallback ? $yield['source_name'].' + '.$groqFallback['source_name'] : $yield['source_name'],
            'source_url' => null,
            'source_credit' => $groqFallback
                ? 'Prediction values came from trained iClimate models/rules; explanation was assisted by Groq.'
                : 'Prediction values came from trained iClimate models and local decision-support rules.',
            'language' => $language,
            'memory' => $memory,
            'prediction_result' => [
                ...
                'generative_ai' => $groqFallback ? [
                    ...
                    'reason' => 'Trained model output was unavailable or low-confidence, so Groq explained the iClimate result.',
                ] : null,
    ...
    private function validationResponse(string $intent, string $language, array $memory, float $startedAt, string $message, CarbonImmutable $targetDate): array
    {
        return [
            'intent' => $intent,
            'answer' => $message,
            'source_type' => 'Validation',
            'source_name' => 'iClimate Date Validation',
            'source_url' => null,
            'source_credit' => 'Source: iClimate Date Validation',
            'language' => $language,
            'memory' => [
                ...$memory,
                'last_answer_source' => 'Validation',
    ...
    private function parseTargetDateFromQuestion(string $question): ?CarbonImmutable
    {
        ...
        if (str_contains($text, 'this month') || str_contains($text, 'ngayong buwan')) {
            return CarbonImmutable::now();
        }

        if (preg_match('/\b(\d{4})-(\d{1,2})(?:-(\d{1,2}))?\b/', $question, $match)) {
            try {
                return CarbonImmutable::createFromDate((int) $match[1], (int) $match[2], (int) ($match[3] ?? 1));
            } catch (\Throwable) {
                return null;
            }
        }
        ...
        if (preg_match('/\b('.implode('|', array_keys($months)).')\b(?:\s+(\d{1,2})(?!\d))?,?\s*(\d{4})?/i', $text, $match)) {
            $month = $months[strtolower($match[1])];
            $day = ! empty($match[2]) ? (int) $match[2] : 1;
            $hasYear = ! empty($match[3]);
            $year = $hasYear ? (int) $match[3] : CarbonImmutable::now()->year;
            ...
            if (! $hasYear && $date->lessThanOrEqualTo(CarbonImmutable::today())) {
                $date = $date->addYear();
            }
    ...
    private function groqPredictionFallback(User $user, string $question, string $intent, string $language, array $memory, array $context, array $weather, array $yield, array $decision, array $warnings, array $quality, ?string $apiError): ?array
    {
        if (! $this->shouldAskGroqForPredictionFallback($quality, $apiError, (bool) ($yield['fallback'] ?? false))) {
            return null;
        }
        ...
        return $this->groqChat->predictionFallback($user, $question, $intent, $language, $memory, [
            ...
            'fallback_used' => $apiError !== null || (($yield['fallback'] ?? false) === true),
        ]);
    }

    private function shouldAskGroqForPredictionFallback(array $quality, ?string $apiError, bool $isFallback): bool
    {
    ...
    private function textResponse(string $answer, string $intent, string $language, array $memory, float $startedAt, array $metadata): array
    {
        ...
        return [
            ...
            'source_credit' => $metadata['source_credit'] ?? ('Source: '.$sourceName),
            'language' => $language,
            'memory' => [
                ...$memory,
    ...
    private function sourceNotes(array $numbers, array $conditions, CarbonImmutable $targetMonth): array
    {
        $notes = [];
        ...
        $notes[] = 'Missing weather values used the iClimate Random Forest model prediction for '.$targetMonth->format('F Y').'.';
    ...
    private function predictionQuality(array $context, array $yield, ?string $apiError): array
    {
        $issues = [];

        if ($apiError) {
            $issues[] = 'Trained model API unavailable; fallback rules were used.';
        }

        if (($yield['fallback'] ?? false) === true) {
            $issues[] = 'Trained model was unavailable; a local rule-based estimate was used.';
        }
        ...
        $score = max(35, min(95, 100 - (count($issues) * 12)));
    ...
    private function adjustedConfidence(array $yield, array $decision, array $quality): float
    {
        $base = (float) ($yield['confidence'] ?? $decision['confidence']['value'] ?? 65);
    ...
    private function riskLabel(string $label, string $language): string
    {
        ...
        return match (strtolower($label)) {
            'severe risk' => 'Matinding Panganib',
            'high risk' => 'Mataas na Panganib',
            'moderate risk', 'medium risk' => 'Katamtamang Panganib',
